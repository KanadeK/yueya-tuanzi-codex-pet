...
def validate_pet(root: Path = ROOT) -> dict[str, Any]:
    ...
    if metadata.get("id") != "yuexin-miao":
        errors.append("pet.json id must be yuexin-miao")
    if metadata.get("displayName") != "月薪喵":
        errors.append("pet.json displayName must be 月薪喵")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate the Yuexin Miao Codex pet package")
